...
{
    @Query("update Horse set allowed_daily_feedings = :allowedDailyFeedings where id= :id")
    @Modifying
    ...
    @Query("update Horse set alias = :alias where id= :id")
    @Modifying
    void updateAlias(@Param("id") Long id, @Param("alias") String alias);

    @Query("update Horse set owner_name = :owner_name where id= :id")
    @Modifying
